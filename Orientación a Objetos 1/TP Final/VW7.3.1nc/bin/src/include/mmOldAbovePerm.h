/* SccsId is  @(#)mmOldAbovePerm.h	58.10 04/06/05 */
/************************************************************************
 * File: mmOldAbovePerm.h
 *
 * This file contains the memory manager defs that are dependent on oldSpace
 * being located above permSpace in memory. See mmOldBelowPerm.h for alternate
 * implementations of these defs appropriate for platforms where oldSpace is
 * located above permSpace in memory. This file should be include by platform.h.
 *
...
#define oldAbovePerm TRUE
#define mappedPermSpace FALSE
...
#define dataPointerInSegmentedSpace(ptr) aboveOldPermBoundary(ptr)
...
#define objIsOld(objOop) aboveOldPermBoundary(objOop)
#define objIsOldAssert(objOop) aboveOldPermBoundary(objOop)
...
# define nonNewObjIsPerm(oldObj)   (assert((oldObj) >= (oop)eden.boundary), \
									belowOldPermBoundary(oldObj))
#else
# define nonNewObjIsPerm(oldObj) belowOldPermBoundary(oldObj)
#endif
